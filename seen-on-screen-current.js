(()=>{
  ...
  async function current(){try{const r=await fetch(API,{cache:'no-store'});if(!r.ok)return[];const d=await r.json();return Array.isArray(d.items)?d.items:[]}catch{return[]}}
  async function clean(){const sec=section();if(!sec)return false;const items=await current();if(!items.length)return false;const want=new Set(items.map(x=>x.slug));const present=new Set(cards(sec).map(slugFrom).filter(Boolean));if(!items.every(x=>present.has(x.slug)))return false;
    ...
    const r=rail(sec);if(r){for(const item of items){const c=cards(sec).find(x=>slugFrom(x)===item.slug);if(c&&c.parentElement===r)r.appendChild(c)}}
