(()=>{
  if(window.__vyrdictProductNavMarketV1)return;
  window.__vyrdictProductNavMarketV1=1;

  const DETAIL='https://shmbvkjzeqqxybweyowj.supabase.co/functions/v1/vyrdict-product-detail';
  const STYLE_ID='vyrdict-product-nav-market-style';
  const PANEL_CLASS='vyrdict-market-links-v1';
  const HOME_CLASS='vyrdict-product-home-v1';
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      .${PANEL_CLASS} .vyrdict-market-missing{font:700 10px/1.35 Arial,Helvetica,sans-serif;color:#857a70}
  ...
  function homeLink(){
    ...
    if(!host)return;
    const a=document.createElement('a');a.href='/';a.className=HOME_CLASS;a.dataset.vyrdictHome='1';a.textContent='← Home';
    host.prepend(a);
  }

  function productId(){
    const s=slug();if(!s)return null;
    try{if(typeof S!=='undefined'&&Array.isArray(S.p)){const p=S.p.find(x=>String(x?.slug||'')===s);if(p?.id)return Number(p.id)}}catch{}
  ...
  function renderMarket(buy,rows){
    ...
    for(const code of ['US','CA']){
      ...
      if(market.length){
        market.slice(0,3).forEach(r=>{const a=document.createElement('a');a.href=validUrl(r);a.target='_blank';a.rel='noopener sponsored';a.innerHTML=`<span>${String(r.retailer_name||'Shop').replace(/</g,'&lt;')}</span><span>↗</span>`;list.appendChild(a)})
      }else{
        const m=document.createElement('div');m.className='vyrdict-market-missing';m.textContent=`${code==='US'?'U.S.':'Canada'} availability is not yet verified.`;list.appendChild(m)
      }
