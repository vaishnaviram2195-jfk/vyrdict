(()=>{
  if(window.__vyrdictProductFastV1)return;
  window.__vyrdictProductFastV1=1;
  const ENDPOINT='https://shmbvkjzeqqxybweyowj.supabase.co/functions/v1/vyrdict-product-detail';
  const inflight=new Map();
  const TTL=10*60*1000;

  function cacheKey(id){return `vyrdict:product-detail:v1:${id}`}
  function readCache(id){
    try{const x=JSON.parse(sessionStorage.getItem(cacheKey(id))||'null');if(x?.data&&Date.now()-Number(x.ts||0)<TTL)return x.data}catch{}
    return null;
  }
  function writeCache(id,data){try{sessionStorage.setItem(cacheKey(id),JSON.stringify({ts:Date.now(),data}))}catch{}}
  ...
  function install(attempt=0){
    if(typeof api!=='function'){if(attempt<30)setTimeout(()=>install(attempt+1),40);return}
  ...
  function prefetch(el){
    const b=el?.closest?.('[data-product]');if(!b)return;
    const slug=String(b.dataset.product||'');if(!slug)return;
    try{if(typeof S!=='undefined'&&Array.isArray(S.p)){const p=S.p.find(x=>x.slug===slug);if(p?.id)getDetail(p.id).catch(()=>{})}}catch{}
  }
  ...
  document.addEventListener('touchstart',e=>prefetch(e.target),{capture:true,passive:true});
  install();
})();
