...
function buildTitle(fullName,question){
  ...
  const review=`${fullName} Review | VYRDICT`;
  if(review.length<=60)return review;
  const suffix=' Review | VYRDICT';
  const base=trimAtWord(String(fullName||'').replace(/\s+[—–]\s+.*$/,'').trim(),60-suffix.length);
  return`${base}${suffix}`;
}
const transitionCss=`#vyrdict-initial-cover{position:fixed;inset:0;z-index:2147483647;background:#f4ede5;display:grid;place-items:center;font-family:Arial,Helvetica,sans-serif;color:#171511}#vyrdict-initial-cover>div{width:min(420px,calc(100% - 40px));background:#fffdf8;border:1px solid #d8cec4;border-radius:24px;padding:28px;text-align:center;box-shadow:0 18px 55px rgba(58,43,32,.08)}#vyrdict-initial-cover .vlogo{font:950 27px/1 Arial,Helvetica,sans-serif;letter-spacing:-1.6px;display:inline-flex;align-items:flex-end}#vyrdict-initial-cover .vdot{width:7px;height:7px;background:#e65f72;display:inline-block;margin:0 0 2px 2px}#vyrdict-initial-cover .vloading{margin-top:14px;font:800 10px/1.4 Arial,Helvetica,sans-serif;letter-spacing:.08em;text-transform:uppercase;color:#6d675f}`;
const transitionHtml=`<div id="vyrdict-initial-cover" role="status" aria-live="polite"><div><div class="vlogo">VYRDICT<span class="vdot"></span></div><div class="vloading">Loading the verdict…</div></div></div>`;
...
module.exports=async function handler(req,res){
  try{
    ...
    const description=trimAtWord(`${fullName}: Viral ${p.viral_score}/100, Worth ${p.worth_score}/100 — ${p.verdict}. ${verdict}`,158);
    ...
    if(isBook){if(brand)entity.author={'@type':'Person',name:brand}}else{
      entity.category=p.category||'';if(brand)entity.brand={'@type':'Brand',name:brand};
      if(pricedRetailers.length){
        const offers=pricedRetailers.slice(0,3).map(r=>({'@type':'Offer',url:r.retailer_url,price:Number(r.price),priceCurrency:String(r.currency).toUpperCase(),availability:'https://schema.org/InStock',itemCondition:'https://schema.org/NewCondition'}));
        entity.offers=offers.length===1?offers[0]:offers;
      }
    }
...
function notFound(res){
  ...
  return res.status(404).send('<!doctype html><html><head><meta name="robots" content="noindex,follow"><title>Product not found | VYRDICT</title><script src="/home-return-failsafe.js?v=1" defer></script><script src="/navigation-guard.js?v=1" defer></script></head><body><h1>Product not found</h1><a href="/">Back to VYRDICT</a></body></html>');
}