(()=>{
  ...
  const STYLE_ID='vyrdict-trending-gallery-v1-style';
  const norm=s=>String(s||'').toLowerCase().replace(/[’‘]/g,"'").replace(/&/g,' and ').replace(/[^a-z0-9]+/g,' ').trim();
  const esc=v=>String(v??'').replace(/[&<>'"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[m]));
  const isHome=()=>location.pathname==='/'||location.pathname==='';
  let section=null,allProducts=[],products=[],activeCategory='All',activeIndex=0,autoTimer=0,observer=null,dragX=null,hovering=false;
  ...
  function addStyle(){
    ...
    s.id=STYLE_ID;
    s.textContent=`
      .vyrdict-index-gallery{--stone:#e6e0d7;--stone-light:#ebe6de;--ink:#171511;--muted:#777069;position:relative;overflow:hidden;background:var(--stone)!important;padding:58px 0 34px!important;border:0!important;color:var(--ink)}
      ...
      .vyrdict-index-gallery .vtg-cat:after{content:'';position:absolute;left:0;right:0;bottom:2px;height:1px;background:var(--ink);transform:scaleX(0);transform-origin:center;transition:transform .28s ease}
      .vyrdict-index-gallery .vtg-cat.is-active{color:var(--ink)}
      .vyrdict-index-gallery .vtg-cat.is-active:after{transform:scaleX(1)}

      .vyrdict-index-gallery .vtg-stage{position:relative;height:520px;max-width:1180px;margin:0 auto;touch-action:pan-y;transition:opacity .18s ease,transform .18s ease}
      .vyrdict-index-gallery .vtg-stage.is-switching{opacity:.42;transform:translateY(4px)}
      ...
      .vyrdict-index-gallery .vtg-items{position:absolute;inset:0;z-index:2;overflow:visible}
      .vyrdict-index-gallery .vtg-item{--x:0px;--y:0px;--scale:1;--opacity:1;position:absolute;left:50%;top:50%;width:270px;height:330px;margin:-183px 0 0 -135px;border:0;padding:0;background:transparent;display:flex;align-items:center;justify-content:center;transform:translate(var(--x),var(--y)) scale(var(--scale));opacity:var(--opacity);transition:transform .7s cubic-bezier(.22,.78,.18,1),opacity .38s ease,filter .38s ease;cursor:pointer;will-change:transform,opacity;filter:drop-shadow(0 18px 18px rgba(70,53,43,.11));outline:0}
      .vyrdict-index-gallery .vtg-item img{display:block;max-width:100%;max-height:100%;width:auto;height:auto;object-fit:contain;image-rendering:auto;user-select:none;-webkit-user-drag:none;pointer-events:none}
      .vyrdict-index-gallery .vtg-item[data-rel="0"]{z-index:5;filter:drop-shadow(0 26px 25px rgba(70,53,43,.16))}
      ...
      .vyrdict-index-gallery .vtg-arrow{position:absolute;z-index:8;top:50%;width:42px;height:42px;margin-top:-21px;border:1px solid rgba(23,21,17,.16);border-radius:50%;background:rgba(241,237,231,.72);backdrop-filter:blur(5px);display:grid;place-items:center;color:#2f2c28;font:400 19px/1 Arial,sans-serif;cursor:pointer;transition:background .2s ease,transform .2s ease}
      ...
      @media(max-width:800px){
        ...
        .vyrdict-index-gallery .vtg-stage{height:448px}
        ...
        .vyrdict-index-gallery .vtg-item{width:220px;height:270px;margin:-153px 0 0 -110px}
      ...
      @media(prefers-reduced-motion:reduce){.vyrdict-index-gallery *{scroll-behavior:auto!important;transition-duration:.01ms!important;animation-duration:.01ms!important}}
    `;
  ...
  function buildItems(){
    ...
    products.forEach((p,i)=>{
      ...
      b.addEventListener('click',()=>{
        ...
        activeIndex=i;positionItems();restartAuto();
      });
      box.appendChild(b);
    });
    positionItems();
  ...
  function move(delta){
    ...
    positionItems();restartAuto();
  }

  function startAuto(){
    clearInterval(autoTimer);
    if(products.length<2||window.matchMedia?.('(prefers-reduced-motion: reduce)').matches)return;
    autoTimer=setInterval(()=>{if(!hovering&&document.visibilityState==='visible'){activeIndex=(activeIndex+1)%products.length;positionItems()}},5600);
  }
  ...
  async function selectCategory(cat,weeklyRows=null){
    ...
    setTimeout(()=>{if(!section?.isConnected)return;buildItems();stage?.classList.remove('is-switching');startAuto()},150);
  }

  function bind(){
    section.querySelectorAll('.vtg-cat').forEach(b=>b.addEventListener('click',()=>selectCategory(b.dataset.cat||'All')));
    section.querySelector('.vtg-prev')?.addEventListener('click',()=>move(-1));
    section.querySelector('.vtg-next')?.addEventListener('click',()=>move(1));
    ...
    stage?.addEventListener('pointerup',e=>{if(dragX==null)return;const dx=e.clientX-dragX;dragX=null;if(Math.abs(dx)>42)move(dx<0?1:-1)});
  ...
  async function mount(){
    ...
    const rows=await weekly();allProducts=rows;products=mergeProducts(rows,'All');buildItems();startAuto();
    return true;
  }
