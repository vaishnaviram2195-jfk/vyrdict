(()=>{
  if(window.__vyrdictTrendingGalleryV2)return;
  window.__vyrdictTrendingGalleryV2=1;
  ...
  const STYLE_ID='vyrdict-trending-gallery-v2-style';
  const norm=s=>String(s||'').toLowerCase().replace(/[’‘]/g,"'").replace(/&/g,' and ').replace(/[^a-z0-9]+/g,' ').trim();
  const esc=v=>String(v??'').replace(/[&<>'"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[m]));
  const isHome=()=>location.pathname==='/'||location.pathname==='';
  const sleep=ms=>new Promise(r=>setTimeout(r,ms));
  let section=null,products=[],activeCategory='All',activeIndex=0,autoTimer=0,observer=null,dragX=null,hovering=false,animating=false,animationRun=0;
  ...
  function addStyle(){
    ...
    s.textContent=`
      ...
      .vyrdict-index-gallery .vtg-stage{position:relative;height:520px;max-width:1180px;margin:0 auto;touch-action:pan-y;perspective:1200px;transform-style:preserve-3d;transition:opacity .14s ease,transform .14s ease}
      .vyrdict-index-gallery .vtg-stage.is-switching{opacity:.38;transform:translateY(3px)}
      ...
      .vyrdict-index-gallery .vtg-item{--x:0px;--y:0px;--scale:1;--opacity:1;position:absolute;left:50%;top:50%;width:270px;height:330px;margin:-183px 0 0 -135px;border:0;padding:0;background:transparent;display:flex;align-items:center;justify-content:center;transform:translate3d(var(--x),var(--y),0) scale(var(--scale));opacity:var(--opacity);transition:transform .5s cubic-bezier(.2,.82,.18,1),opacity .3s ease,filter .3s ease;cursor:pointer;will-change:transform,opacity,filter;filter:drop-shadow(0 18px 18px rgba(70,53,43,.11));outline:0;transform-style:preserve-3d;backface-visibility:visible}
      .vyrdict-index-gallery .vtg-item img{display:block;max-width:100%;max-height:100%;width:auto;height:auto;object-fit:contain;image-rendering:auto;user-select:none;-webkit-user-drag:none;pointer-events:none;backface-visibility:hidden}
      .vyrdict-index-gallery .vtg-item[data-rel="0"]{z-index:5;filter:drop-shadow(0 28px 28px rgba(70,53,43,.18))}
      ...
      .vyrdict-index-gallery .vtg-item.is-center{cursor:pointer}
      .vyrdict-index-gallery .vtg-item.is-center:after{content:'VIEW VYRDICT';position:absolute;left:50%;bottom:-14px;transform:translateX(-50%) translateY(4px);padding:7px 10px;border-radius:999px;background:rgba(247,243,237,.86);color:#2f2b27;font:900 7px/1 Arial,Helvetica,sans-serif;letter-spacing:.09em;opacity:0;transition:.18s ease;white-space:nowrap}
      ...
      .vyrdict-index-gallery .vtg-item.vtg-zoom-out{z-index:12!important;pointer-events:none;transition:none!important;animation:vtgZoomOut .18s cubic-bezier(.15,.82,.24,1) forwards}
      .vyrdict-index-gallery .vtg-item.vtg-spiral-next{z-index:12!important;pointer-events:none;transition:none!important;animation:vtgSpiralNext .48s cubic-bezier(.32,.04,.2,1) forwards}
      .vyrdict-index-gallery .vtg-item.vtg-spiral-prev{z-index:12!important;pointer-events:none;transition:none!important;animation:vtgSpiralPrev .48s cubic-bezier(.32,.04,.2,1) forwards}
      .vyrdict-index-gallery .vtg-item.vtg-arrive-next{z-index:11!important;transition:none!important;animation:vtgArriveNext .52s cubic-bezier(.12,.84,.18,1) both}
      .vyrdict-index-gallery .vtg-item.vtg-arrive-prev{z-index:11!important;transition:none!important;animation:vtgArrivePrev .52s cubic-bezier(.12,.84,.18,1) both}
      @keyframes vtgZoomOut{
        from{transform:translate3d(0,0,0) scale(1);opacity:1}
        to{transform:translate3d(0,-7px,88px) scale(1.3);opacity:1}
      }
      @keyframes vtgSpiralNext{
        0%{transform:translate3d(0,-7px,88px) scale(1.3) rotateY(0deg) rotateZ(0deg);opacity:1}
        38%{transform:translate3d(-18px,-10px,60px) scale(1.2) rotateY(78deg) rotateZ(-5deg);opacity:.96}
        72%{transform:translate3d(-88px,9px,-25px) scale(.78) rotateY(188deg) rotateZ(-11deg);opacity:.64}
        100%{transform:translate3d(-250px,28px,-80px) scale(.52) rotateY(315deg) rotateZ(-16deg);opacity:.18}
      }
      @keyframes vtgSpiralPrev{
        0%{transform:translate3d(0,-7px,88px) scale(1.3) rotateY(0deg) rotateZ(0deg);opacity:1}
        38%{transform:translate3d(18px,-10px,60px) scale(1.2) rotateY(-78deg) rotateZ(5deg);opacity:.96}
        72%{transform:translate3d(88px,9px,-25px) scale(.78) rotateY(-188deg) rotateZ(11deg);opacity:.64}
        100%{transform:translate3d(250px,28px,-80px) scale(.52) rotateY(-315deg) rotateZ(16deg);opacity:.18}
      }
      @keyframes vtgArriveNext{
        0%{transform:translate3d(255px,24px,-65px) scale(.5) rotateY(-205deg) rotateZ(10deg);opacity:.18}
        44%{transform:translate3d(58px,-8px,44px) scale(.88) rotateY(-72deg) rotateZ(4deg);opacity:.88}
        78%{transform:translate3d(0,-5px,80px) scale(1.26) rotateY(5deg) rotateZ(-1deg);opacity:1}
        100%{transform:translate3d(0,0,0) scale(1) rotateY(0) rotateZ(0);opacity:1}
      }
      @keyframes vtgArrivePrev{
        0%{transform:translate3d(-255px,24px,-65px) scale(.5) rotateY(205deg) rotateZ(-10deg);opacity:.18}
        44%{transform:translate3d(-58px,-8px,44px) scale(.88) rotateY(72deg) rotateZ(-4deg);opacity:.88}
        78%{transform:translate3d(0,-5px,80px) scale(1.26) rotateY(-5deg) rotateZ(1deg);opacity:1}
        100%{transform:translate3d(0,0,0) scale(1) rotateY(0) rotateZ(0);opacity:1}
      ...
      .vyrdict-index-gallery .vtg-arrow{position:absolute;z-index:20;top:50%;width:42px;height:42px;margin-top:-21px;border:1px solid rgba(23,21,17,.16);border-radius:50%;background:rgba(241,237,231,.72);backdrop-filter:blur(5px);display:grid;place-items:center;color:#2f2c28;font:400 19px/1 Arial,sans-serif;cursor:pointer;transition:background .18s ease,transform .18s ease}
      .vyrdict-index-gallery .vtg-arrow:hover{background:rgba(248,245,240,.95);transform:scale(1.04)}
      ...
      @media(max-width:800px){
        ...
        .vyrdict-index-gallery .vtg-item{width:220px;height:270px;margin:-153px 0 0 -110px;transition-duration:.44s}
        .vyrdict-index-gallery .vtg-arrow{width:38px;height:38px;margin-top:-19px;background:rgba(241,237,231,.82)}
      ...
      @media(prefers-reduced-motion:reduce){
        .vyrdict-index-gallery *{scroll-behavior:auto!important;transition-duration:.01ms!important;animation-duration:.01ms!important}
      }
    `;
  ...
  function mergeProducts(weekly,cat){
    const base=catalog().filter(p=>p&&p.slug&&p.image_url);
    const ranked=(weekly||[]).filter(p=>p&&p.slug&&p.image_url);
    ...
    const diversified=[],seenCats=new Set();
    for(const p of list){const c=norm(p.category);if(c&&!seenCats.has(c)){seenCats.add(c);diversified.push(p)}if(diversified.length>=10)break}
    if(diversified.length<7){for(const p of list){if(!diversified.some(x=>x.slug===p.slug))diversified.push(p);if(diversified.length>=10)break}}
  ...
  async function weekly(){
    try{
      ...
      return arr.map(unwrapWeekly).filter(Boolean);
    }catch{return []}
  }
  ...
  function xFor(rel){
    const w=section?.querySelector('.vtg-stage')?.clientWidth||1000;
    const step=Math.max(228,Math.min(345,w*.30));
    return rel*step;
  }

  function positionItems(){
    if(!section||!products.length)return;
    const items=[...section.querySelectorAll('.vtg-item')],n=products.length;
    items.forEach((el,i)=>{
      if(el.classList.contains('vtg-zoom-out')||el.classList.contains('vtg-spiral-next')||el.classList.contains('vtg-spiral-prev')||el.classList.contains('vtg-arrive-next')||el.classList.contains('vtg-arrive-prev'))return;
      ...
      let scale=1,opacity=1,y=0;
      if(abs===1){scale=.66;opacity=.82;y=18}
      else if(abs===2){scale=.46;opacity=.28;y=42}
      else if(abs>2){scale=.34;opacity=0;y=52}
      el.style.setProperty('--x',`${xFor(rel)}px`);
      el.style.setProperty('--y',`${y}px`);
      el.style.setProperty('--scale',String(scale));
      el.style.setProperty('--opacity',String(opacity));
      el.tabIndex=abs<=1?0:-1;
      el.setAttribute('aria-label',rel===0?`Open VYRDICT for ${(products[i].brand?products[i].brand+' ':'')+products[i].name}`:`Bring ${products[i].name} into focus`);
    });
  ...
  function clearMotionClasses(){
    section?.querySelectorAll('.vtg-item').forEach(el=>el.classList.remove('vtg-zoom-out','vtg-spiral-next','vtg-spiral-prev','vtg-arrive-next','vtg-arrive-prev'));
  }

  async function animateMove(delta,manual=false){
    if(animating||products.length<2)return;
    if(window.matchMedia?.('(prefers-reduced-motion: reduce)').matches){
      activeIndex=(activeIndex+delta+products.length)%products.length;positionItems();restartAuto();return;
    }
    animating=true;
    const run=++animationRun;
    const items=[...section.querySelectorAll('.vtg-item')];
    const oldIndex=activeIndex;
    const newIndex=(activeIndex+delta+products.length)%products.length;
    const oldEl=items[oldIndex],newEl=items[newIndex];
    clearMotionClasses();
    if(oldEl){
      oldEl.classList.add('vtg-zoom-out');
      await sleep(170);
      if(run!==animationRun){animating=false;return}
      oldEl.classList.remove('vtg-zoom-out');
      oldEl.classList.add(delta>0?'vtg-spiral-next':'vtg-spiral-prev');
    }
    await sleep(265);
    if(run!==animationRun){animating=false;return}
    ...
    if(newEl){
      ...
      newEl.classList.add(delta>0?'vtg-arrive-next':'vtg-arrive-prev');
    }
    await sleep(500);
    if(run!==animationRun){animating=false;return}
    clearMotionClasses();
  ...
  function buildItems(){
    const box=section?.querySelector('.vtg-items');
    if(!box)return;
    box.replaceChildren();
    if(!products.length){box.innerHTML='<div class="vtg-empty">No current VYRDICT products are available in this category yet.</div>';section.querySelector('.vtg-count').textContent='';return}
  ...
  function startAuto(){
    ...
    autoTimer=setInterval(()=>{if(!hovering&&!animating&&document.visibilityState==='visible')animateMove(1,false)},4000);
  }
  function restartAuto(){startAuto()}

  async function selectCategory(cat,weeklyRows=null){
    animationRun++;animating=false;clearMotionClasses();
    ...
    const rows=weeklyRows||await weekly();
    products=mergeProducts(rows,cat);
    setTimeout(()=>{if(!section?.isConnected)return;buildItems();stage?.classList.remove('is-switching');startAuto()},120);
  }
  ...
  function bind(){
    ...
    stage?.addEventListener('mouseenter',()=>hovering=true);stage?.addEventListener('mouseleave',()=>hovering=false);
    stage?.addEventListener('focusin',()=>hovering=true);stage?.addEventListener('focusout',()=>hovering=false);
  ...
  async function mount(){
    ...
    clearInterval(autoTimer);addStyle();
    old.className='section vyrdict-index-gallery';old.id='trending-index';old.innerHTML=template();section=old;bind();
    const rows=await weekly();products=mergeProducts(rows,'All');buildItems();startAuto();
    return true;
  }

  function boot(attempt=0){
    Promise.resolve(mount()).then(ok=>{if(!ok&&attempt<35)setTimeout(()=>boot(attempt+1),100)});
  }
